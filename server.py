...
def extract_name_from_import_soup(soup: BeautifulSoup) -> str | None:
  ...
  for node in candidates:
    if not node:
      continue
    content = clean_import_text(node.get("content", ""), 180)
    if content:
      return content

  h1 = soup.find("h1")
  if h1:
    heading = clean_import_text(h1.get_text(" ", strip=True), 180)
    if heading:
      return heading

  title_node = soup.find("title")
  if title_node:
    title = clean_import_text(title_node.get_text(" ", strip=True), 180)
    if title:
      return title
  return None
...
def upsert_imported_clinic_record(
  ...
  with get_db() as conn:
    ...
    if existing:
      ...
      db_name = extracted_name or clean_import_text(existing["name"], 180) or canonical_domain
      conn.execute(
        """
        UPDATE clinics
        SET
